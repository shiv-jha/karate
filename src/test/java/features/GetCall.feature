...
Scenario Outline: Make a POST request  with Authorization Header and validate status code as 201
    Given path '/users'
    Then request {"name":"Tom Shadows12","email":"<email>","gender":"male","status":"active"}
    When method post
    Then status 201
    And print response
	
	 Examples:
     | karate.read("file:src/test/java/package1/user_creation.csv")|
